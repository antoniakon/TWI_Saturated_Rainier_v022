...
//    val rng = ScalaRNG(3)
//    val (dataMap, dataWithInter) = dataGeneration(rng, n1, n2)
...
//  } yield (MyStructure(mu, Nil, Nil, sigE1, sigE2, sigD), List(MyStructure(mu, Nil, Nil, sigE1, sigE2, sigD)))
//
//  /**
//    * Add the main effects of alpha
//    */
//  def addAplha(current: RandomVariable[(MyStructure, List[MyStructure])], i: Int): RandomVariable[(MyStructure, List[MyStructure])] = {
//    for {
//      curr <- current
//      cur = curr._1
//      latestStates = curr._2
//      gm_1 <- Normal(0, cur.sigE1).param
//    } yield (MyStructure(cur.mu, gm_1 :: cur.eff1, cur.eff2, cur.sigE1, cur.sigE2, cur.sigD), latestStates)
//  }
//
//  /**
//    * Add the main effects of beta
//    */
//  def addBeta(current: RandomVariable[(MyStructure, List[MyStructure])], j: Int): RandomVariable[(MyStructure, List[MyStructure])] = {
//    for {
//      curr <- current
//      cur = curr._1
//      latestStates = curr._2
...
//    */
//  def addGroup(current: RandomVariable[(MyStructure, List[MyStructure])], i: Int, j: Int, dataMap: Map[(Int, Int), Vector[Double]]): RandomVariable[(MyStructure, List[MyStructure])] = {
//    for {
//      curr <- current
//      cur = curr._1
//      latestStates = curr._2
//      _ <- Normal(cur.mu + cur.eff1(i) + cur.eff2(j), cur.sigD).fit(dataMap(i, j))
//    } yield (cur, cur::latestStates)
//
//  }
//
//  /**
...
//    */
//  @tailrec def addAllEffRecursive(alphabeta: RandomVariable[(MyStructure, List[MyStructure])], dataMap: Map[(Int, Int), Vector[Double]], i: Int, j: Int): RandomVariable[(MyStructure, List[MyStructure])] = {
//
//    val n1 = alphabeta.value._1.eff1.length
//    val n2 = alphabeta.value._1.eff2.length
//
...
//    */
//  def fullModel(alphabeta: RandomVariable[(MyStructure, List[MyStructure])], dataMap: Map[(Int, Int), Vector[Double]]) : RandomVariable[(MyStructure, List[MyStructure])] = {
...
//      static = tup._1
//      states = tup._2
//    } yield Map("mu" -> static.mu,
//     "states" -> states.head.mu)
//
//    // sampling
